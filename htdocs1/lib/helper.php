		...
		if($stmt->num_rows==1){
			...
			if(check_brute($user_id,$mysqli)==TRUE){
				...
				if($password_bd==$password){
					...
					$_SESSION['login_string']=hash('sha512',$password.$user_agent_browser);
					return true;
				}
				else{
					$now=time();
...
function redirect_page($page){
	...
	if($curr_file!=$page.'.php'){
		header("Location: ".CUR_WEBPATH.$page.".php");
		exit();
	}	
}
function logout_user(){
	...
	session_destroy();
	header("Location: ".CUR_WEBPATH."login.php");
	exit();
}
